    ...
    switch (type) {
        ...
        case "i8":
            ...
            if (type[type.length - 1] === "*") {
                ...
                assert(bits % 8 === 0, "getNativeTypeSize invalid bits " + bits + ", type " + type);return bits / 8
            } else {
                return 0
            }
        }
...
if (ENVIRONMENT_IS_NODE) {
    ...
    function UTF8ArrayToString(u8Array, idx, maxBytesToRead) {
        ...
        buf = buf.slice(idx, maxBytesToRead > 0 && idx + maxBytesToRead < buf.length ? idx + maxBytesToRead : buf.length);var endPtr = buf.indexOf(0);
...
function stringToUTF8Array(str, outU8Array, outIdx, maxBytesToWrite) {
    ...
    if (ENVIRONMENT_IS_NODE && maxBytesToWrite > 30) {
        ...
        outU8Array[outIdx + length] = 0;return length
    }
    var startIdx = outIdx;
...
function stringToUTF16(str, outPtr, maxBytesToWrite) {
    ...
    maxBytesToWrite -= 2;var startPtr = outPtr;
...
var buffer, HEAPBUFFER, HEAP8, HEAPU8, HEAP16, HEAPU16, HEAP32, HEAPU32, HEAPF32, HEAPF64;
...
INITIAL_INITIAL_MEMORY = buffer.byteLength;updateGlobalBufferAndViews(buffer);
...
function initRuntime() {
    ...
    TTY.init();callRuntimeCallbacks(__ATINIT__)
}
function preMain() {
...
var wasmBinaryFile = "flatbuffers_addon_wasm.wasm";
...
var NODEFS = {
    ...
    stream_ops: {
        ...
        mmap: function(stream, buffer, offset, length, position, prot, flags) {
            ...
            assert(offset === 0);NODEFS.stream_ops.read(stream, buffer, ptr + offset, length, position);return {
                ptr: ptr,
                allocated: true
            }
        },
...
var NODERAWFS = {
    ...
    llseek: function(stream, offset, whence) {
        ...
        stream.position = position;return position
    },
    read: function(stream, buffer, offset, length, position) {
        if (stream.stream_ops) {
        ...
        for (var i = 0; i < parts.length; i++) {
            ...
            if (!islast || opts.follow) {
                ...
                while (FS.isLink(current.mode)) {
                    ...
                    current_path = PATH_FS.resolve(PATH.dirname(current_path), link);var lookup = FS.lookupPath(current_path, {
    ...
    createLazyFile: function(parent, name, url, canRead, canWrite) {
        ...
        LazyUint8Array.prototype.cacheLength = function LazyUint8Array_cacheLength() {
            ...
            if (usesGzip || !datalength) {
                ...
                chunkSize = datalength;out("LazyFiles on gzip forces download of the whole file when length is accessed")
            }
...
var SYSCALLS = {
    ...
    doStat: function(func, path, buf) {
        ...
        HEAP32[buf + 32 >> 2] = 0;tempI64 = [stat.size >>> 0,(tempDouble = stat.size, +Math_abs(tempDouble) >= 1 ? tempDouble > 0 ? (Math_min(+Math_floor(tempDouble / 4294967296), 4294967295) | 0) >>> 0 : ~~+Math_ceil((tempDouble - +(~~tempDouble >>> 0)) / 4294967296) >>> 0 : 0)], HEAP32[buf + 40 >> 2] = tempI64[0], HEAP32[buf + 44 >> 2] = tempI64[1];
        ...
        HEAP32[buf + 76 >> 2] = 0;tempI64 = [stat.ino >>> 0,(tempDouble = stat.ino, +Math_abs(tempDouble) >= 1 ? tempDouble > 0 ? (Math_min(+Math_floor(tempDouble / 4294967296), 4294967295) | 0) >>> 0 : ~~+Math_ceil((tempDouble - +(~~tempDouble >>> 0)) / 4294967296) >>> 0 : 0)], HEAP32[buf + 80 >> 2] = tempI64[0], HEAP32[buf + 84 >> 2] = tempI64[1];return 0
    },
...
function ___syscall221(fd, cmd, varargs) {
    SYSCALLS.varargs = varargs;try {
        ...
        switch (cmd) {
            ...
            case 8:
                return -28;case 9:
                ___setErrNo(28);return -1;default: {
                return -28
            }
        }
    } catch ( e ) {
...
function syscallMunmap(addr, len) {
    ...
    if (len === info.len) {
        ...
        SYSCALLS.doMsync(addr, stream, len, info.flags, info.offset);FS.munmap(stream);
...
function makeLegalFunctionName(name) {
    ...
    name = name.replace(/[^a-zA-Z0-9_]/g, "$");var f = name.charCodeAt(0);
...
function releaseClassHandle($$) {
    $$.count.value -= 1;var toDelete = 0 === $$.count.value;
    if (toDelete) {
        runDestructor($$)
    }
}
...
function ClassHandle_clone() {
    ...
    if (this.$$.preservePointerOnDelete) {
        ...
        clone.$$.count.value += 1;
        clone.$$.deleteScheduled = false;return clone
    }
}
...
function RegisteredPointer_fromWireType(ptr) {
    ...
    if (undefined !== registeredInstance) {
        if (0 === registeredInstance.$$.count.value) {
            ...
            registeredInstance.$$.smartPtr = ptr;return registeredInstance["clone"]()
        } else {
            var rv = registeredInstance["clone"]();
            this.destructor(ptr);return rv
        }
    }
...
function __embind_register_class(rawType, rawPointerType, rawConstPointerType, baseClassRawType, getActualTypeSignature, getActualType, upcastSignature, upcast, downcastSignature, downcast, name, destructorSignature, rawDestructor) {
    ...
    rawDestructor = embind__requireFunction(destructorSignature, rawDestructor);var legalFunctionName = makeLegalFunctionName(name);
    exposePublicSymbol(legalFunctionName, function() {
        ...
        };replacePublicSymbol(legalFunctionName, constructor);return [referenceConverter, pointerConverter, constPointerConverter]
    })
}
...
function __embind_register_class_constructor(rawClassType, argCount, rawArgTypesAddr, invokerSignature, invoker, rawConstructor) {
    assert(argCount > 0);var rawArgTypes = heap32VectorToArray(argCount, rawArgTypesAddr);
    invoker = embind__requireFunction(invokerSignature, invoker);var args = [rawConstructor];
    var destructors = [];
    whenDependentTypesAreResolved([], [rawClassType], function(classType) {
        ...
        classType.registeredClass.constructor_body[argCount - 1] = function unboundTypeHandler() {
            ...
            classType.registeredClass.constructor_body[argCount - 1] = function constructor_body() {
                ...
                runDestructors(destructors);return argTypes[0]["fromWireType"](ptr)
            };return []
        });return []
    })
}
...
function craftInvokerFunction(humanName, argTypes, classType, cppInvokerFunc, cppTargetFunc) {
    ...
    invokerFnBody += "}\n";args1.push(invokerFnBody);var invokerFunction = new_(Function, args1).apply(null, args2);
    return invokerFunction
}
function __embind_register_class_function(rawClassType, methodName, argCount, rawArgTypesAddr, invokerSignature, rawInvoker, context, isPureVirtual) {
    var rawArgTypes = heap32VectorToArray(argCount, rawArgTypesAddr);
    methodName = readLatin1String(methodName);
    rawInvoker = embind__requireFunction(invokerSignature, rawInvoker);whenDependentTypesAreResolved([], [rawClassType], function(classType) {
        classType = classType[0];
...
function __embind_register_emval(rawType, name) {
    name = readLatin1String(name);registerType(rawType, {
        ...
        "fromWireType": function(handle) {
            ...
            __emval_decref(handle);return rv
        },
...
function __embind_register_enum(rawType, name, size, isSigned) {
    ...
    ctor.values = {};registerType(rawType, {
        name: name,
        constructor: ctor,
...
function __embind_register_enum_value(rawEnumType, name, enumValue) {
    ...
    name = readLatin1String(name);var Enum = enumType.constructor;
...
function __embind_register_function(name, argCount, rawArgTypesAddr, signature, rawInvoker, fn) {
    ...
    name = readLatin1String(name);
    rawInvoker = embind__requireFunction(signature, rawInvoker);exposePublicSymbol(name, function() {
...
function __embind_register_memory_view(rawType, dataTypeIndex, name) {
    ...
    function decodeMemoryView(handle) {
        handle = handle >> 2;var heap = HEAPU32;
...
function __embind_register_std_string(rawType, name) {
    name = readLatin1String(name);var stdStringIsUTF8 = name === "std::string";
...
function __embind_register_std_wstring(rawType, charSize, name) {
    name = readLatin1String(name);var decodeString, encodeString, getHeap, lengthBytesUTF, shift;
    if (charSize === 2) {
    ...
    registerType(rawType, {
        ...
        "toWireType": function(destructors, value) {
            ...
            HEAPU32[ptr >> 2] = length >> shift;encodeString(value, ptr + 4, length + charSize);
            if (destructors !== null) {
                destructors.push(_free, ptr)
            }
            return ptr
...
function __emval_take_value(type, argv) {
    type = requireRegisteredType(type, "_emval_take_value");var v = type["readValueFromPointer"](argv);
...
function _emscripten_resize_heap(requestedSize) {
    ...
    for (var cutDown = 1; cutDown <= 4; cutDown *= 2) {
        ...
        overGrownHeapSize = Math.min(overGrownHeapSize, requestedSize + 100663296);var newSize = Math.min(maxHeapSize, alignUp(Math.max(minHeapSize, requestedSize, overGrownHeapSize), PAGE_MULTIPLE));
...
function _environ_get(__environ, environ_buf) {
    ...
    strings.forEach(function(string, i) {
        ...
        HEAP32[__environ + i * 4 >> 2] = ptr;writeAsciiToMemory(string, ptr);
...
function _fd_seek(fd, offset_low, offset_high, whence, newOffset) {
    try {
        ...
        FS.llseek(stream, offset, whence);tempI64 = [stream.position >>> 0,(tempDouble = stream.position, +Math_abs(tempDouble) >= 1 ? tempDouble > 0 ? (Math_min(+Math_floor(tempDouble / 4294967296), 4294967295) | 0) >>> 0 : ~~+Math_ceil((tempDouble - +(~~tempDouble >>> 0)) / 4294967296) >>> 0 : 0)], HEAP32[newOffset >> 2] = tempI64[0], HEAP32[newOffset + 4 >> 2] = tempI64[1];
...
UnboundTypeError = Module["UnboundTypeError"] = extendError(Error, "UnboundTypeError");init_emval();
...
var dynCall_iii = Module["dynCall_iii"] = function() {
    return (dynCall_iii = Module["dynCall_iii"] = Module["asm"]["X"]).apply(null, arguments)
};
var dynCall_vi = Module["dynCall_vi"] = function() {
    return (dynCall_vi = Module["dynCall_vi"] = Module["asm"]["Y"]).apply(null, arguments)
};
var dynCall_i = Module["dynCall_i"] = function() {
    return (dynCall_i = Module["dynCall_i"] = Module["asm"]["Z"]).apply(null, arguments)
};
var dynCall_viii = Module["dynCall_viii"] = function() {
    return (dynCall_viii = Module["dynCall_viii"] = Module["asm"]["_"]).apply(null, arguments)
};
var dynCall_ii = Module["dynCall_ii"] = function() {
    return (dynCall_ii = Module["dynCall_ii"] = Module["asm"]["$"]).apply(null, arguments)
};
var dynCall_viiii = Module["dynCall_viiii"] = function() {
    return (dynCall_viiii = Module["dynCall_viiii"] = Module["asm"]["aa"]).apply(null, arguments)
};
var dynCall_iiii = Module["dynCall_iiii"] = function() {
    return (dynCall_iiii = Module["dynCall_iiii"] = Module["asm"]["ba"]).apply(null, arguments)
};
...
var dynCall_iiiiiii = Module["dynCall_iiiiiii"] = function() {
    return (dynCall_iiiiiii = Module["dynCall_iiiiiii"] = Module["asm"]["ea"]).apply(null, arguments)
};
var dynCall_iiiiiiiiiii = Module["dynCall_iiiiiiiiiii"] = function() {
    return (dynCall_iiiiiiiiiii = Module["dynCall_iiiiiiiiiii"] = Module["asm"]["fa"]).apply(null, arguments)
};
var dynCall_viiiiiiiiii = Module["dynCall_viiiiiiiiii"] = function() {
    return (dynCall_viiiiiiiiii = Module["dynCall_viiiiiiiiii"] = Module["asm"]["ga"]).apply(null, arguments)
};
var dynCall_iiiii = Module["dynCall_iiiii"] = function() {
    return (dynCall_iiiii = Module["dynCall_iiiii"] = Module["asm"]["ha"]).apply(null, arguments)
};
...
var dynCall_ji = Module["dynCall_ji"] = function() {
    return (dynCall_ji = Module["dynCall_ji"] = Module["asm"]["la"]).apply(null, arguments)
};
var dynCall_iiiiiiiii = Module["dynCall_iiiiiiiii"] = function() {
    return (dynCall_iiiiiiiii = Module["dynCall_iiiiiiiii"] = Module["asm"]["ma"]).apply(null, arguments)
};
...
var dynCall_iiiiid = Module["dynCall_iiiiid"] = function() {
    return (dynCall_iiiiid = Module["dynCall_iiiiid"] = Module["asm"]["ra"]).apply(null, arguments)
};
var dynCall_iiiiijj = Module["dynCall_iiiiijj"] = function() {
    return (dynCall_iiiiijj = Module["dynCall_iiiiijj"] = Module["asm"]["sa"]).apply(null, arguments)
};
var dynCall_viiiii = Module["dynCall_viiiii"] = function() {
    return (dynCall_viiiii = Module["dynCall_viiiii"] = Module["asm"]["ta"]).apply(null, arguments)
};
Module["asm"] = asm;
