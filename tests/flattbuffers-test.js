...
describe("flatbuffers", () => {
    let schema_contents, MyGame, binary;
    ...
    before(cb => {
        waterfall([
            next => {
                fs.readFile(schema, next);
            },

            (buffer, next) => {
                schema_contents = buffer;

                const sandbox = {};
                (new Function(addon.js({ // eslint-disable-line no-new-func
                    schema,
                    schema_contents,
                ...
                MyGame = sandbox.MyGame;

                fs.readFile(json, next);
            },
